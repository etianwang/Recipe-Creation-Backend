...
export function buildRecipeSystemPrompt(): string {
  return [
    ...
    'ingredient.name MUST be exactly one ingredient — never join multiple with +/＋/和. Put quantities only in amount. Wrong: name「八角1颗+桂皮1小段」. Right: two items {name:"八角",amount:"1颗"} and {name:"桂皮",amount:"1小段"}.',
...
export function buildRecipeUserPrompt(
  ...
  return [
    ...
    'One ingredient per list entry; never name like 「八角1颗+桂皮1小段」.',
